...
function initEventListeners(t = !1) {
	adButtons.forEach((t) => {
		t.addEventListener("click", (e) => {
			if (!(adViewed = JSON.parse(localStorage.getItem("popupAD")))) {
				e.preventDefault(), popupAd(t);
				return;
			}
		});
	});
...
function popupAd(t) {
	let e = new URLSearchParams(window.location.search),
		a =
			popupAdConfig.h1 ||
			e.get("h1") ||
			t.getAttribute("data-h1") ||
			"Popup AD",
		o =
			popupAdConfig.button ||
			e.get("btn") ||
			t.getAttribute("data-btn-txt") ||
			"Earn Reward",
		d =
			popupAdConfig.timer ||
			e.get("timer") ||
			t.getAttribute("data-timer") ||
			10,
		i =
		...
		n =
			...
			e.get("mobilead") ||
			t.getAttribute("data-mobilead") ||
			i,
		r =
			popupAdConfig.desktopAd ||
			e.get("desktopad") ||
			t.getAttribute("data-desktopad") ||
			i,
